...
void oauth2::request_user_code(
    ...
    LOG_DEBUG1("Requesting device code from google")
    curl c;
    ...
    LOG_DEBUG2("No error in the response, processing it")
    ...
    LOG_DEBUG1("Device code read")
}

void oauth2::begin_polling() {
    LOG_DEBUG1("Polling for tokens")
    ...
    while (polling_ends > std::chrono::system_clock::now()) {
        ...
        if (root.isMember("error")) {
            ...
            if (error == "authorization_pending") {
                //this is normal
                LOG_DEBUG2("No tokens yet, waiting some more")
            } else if (error == "slow_down") {
                LOG_DEBUG2("We are asking to fast, slowing down a little bit.")
                ++interval_;
            } else {
                LOG_ERROR(
            ...
            LOG_DEBUG1("We've got tokens, breaking loop.")
            return;
        }
...
void oauth2::refresh_access_token() {
    LOG_DEBUG1("Requesting token refresh")
    curl c;
    c.url("https://accounts.google.com/o/oauth2/token");
    c.method(method::POST);
    c.form_field("client_id", config::client_id());
